...
- [ ] T006 Reproduce the bug on the primary device per `specs/025-dashboard-scroll-jump/quickstart.md` "Reproduction" section; capture the baseline recording and save it as `before-android-navbar.mp4` alongside the feature's spec artifacts (or linked path documented in `research.md`)
- [ ] T007 Fill in the "Reproduction baseline" section of `specs/025-dashboard-scroll-jump/research.md` with exact cold-start procedure, the first-observed frame showing the clipped TopNav, and a link to the recording
...
- [ ] T014 [US1] Based on FD-1 in `research.md`: if confirmed, refactor `apps/mobile/app/(tabs)/index.tsx` to render `TopNav` as a **sibling** of `ScrollView` (both children of `StarryBackground`) rather than a child of `ScrollView`. Preserve current styling (`px-5 pt-[10px]`), ensure TopNav receives the same props, and ensure the ScrollView's `contentContainerStyle` accounts for the new layout (add `paddingTop` matching the TopNav height so content does not render underneath it)
- [ ] T015 [US1] If FD-2 confirmed any non-#2 hypothesis as an independent contributor, apply its targeted fix (scope depends on findings — document the specific edit in the task's commit message). If FD-2 ruled out all others, skip this task and note "FD-2 ruled out; skipped" in the commit for T014
- [ ] T016 [US1] If FD-3 concluded a defensive guard is required: create `apps/mobile/hooks/useDashboardLoadingState.ts` — a lifecycle-only hook that aggregates the per-section loading flags (`accountsLoading`, `ratesLoading`, `netWorthLoading`, `transactionsLoading`, and any monthly/upcoming flags present in `index.tsx`) into a single `allSectionsLoaded` boolean matching the clarification Q3 definition. The hook MUST be pure lifecycle/subscription code per constitution IV — no business calculations, no writes
- [ ] T017 [US1] If T016 was created, wire it into `apps/mobile/app/(tabs)/index.tsx`: on the transition from `false → true` of `allSectionsLoaded`, call `scrollRef.current?.scrollTo({ y: 0, animated: false })` **only if** the user has not scrolled (guard with a `userHasScrolledRef` set `true` in the ScrollView's `onScroll` when `contentOffset.y > threshold`). This preserves FR-005. Skip this task if FD-3 was "no"
- [ ] T018 [US1] Add unit tests for `useDashboardLoadingState` in `apps/mobile/hooks/__tests__/useDashboardLoadingState.test.ts` using Jest + React Native Testing Library — verify: (a) returns `allSectionsLoaded=false` while any flag is `true`, (b) returns `allSectionsLoaded=true` only when ALL flags are `false`, (c) transitions correctly when flags flip. Skip this task if T016 was not created
