...
- Local calculations (net worth, account balances, asset valuations) MUST NOT
  depend on API availability. Use `@astik/logic` for on-device computation.
...
- **Schema-driven UI**: All data-driven screens MUST strictly match the existing
  database schema (`@astik/db` models). Do NOT invent, rename, remove, or infer
  fields. Labels, data types, and required/optional states MUST reflect the
  schema exactly.

### VI. Monorepo Package Boundaries

The Astik monorepo uses npm workspaces + Nx with strict dependency direction.

- **`packages/db` (`@astik/db`)**: WatermelonDB models, schema definitions, type
  exports, and sync configuration. MUST NOT import from `apps/` or other
  packages.
- **`packages/logic` (`@astik/logic`)**: Shared business logic (asset
...
- **`apps/api`**: The Express.js backend. May import from `@astik/logic` for
  shared logic.
